...
public class Page_N {
	 ...
	 * 		책의 페이지를 넘기는 것에는 한계가 존재하며 이 한게를 넘는 요청이 발생하는 경우 "더 이상 페이지를 넘길 수 없습니다." 메시지를 출력하도록 한다.
	 * 		위의 사항을 만족할 수 있도록 필요한 모든 생성자, 멤버변수, 메서드 등을 만들고 클래스 다이어그램으로도 만들어 보세요.
	 */
	
	private int pageNumber;
	private int limitPageNumber;
	
	public Page_N(int limitPageNumber) {
	...
	public void nextPage(int number) {
		if(this.existsNextPage()) { // 페이지 존재 유무 확인 후 증가
