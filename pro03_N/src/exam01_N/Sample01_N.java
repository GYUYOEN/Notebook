...
public class Sample01_N {
	...
	public static void main(String[] args) {
		 ...
		 *      - 조건문의 종류는 if, if ... else if, switch 가 있다.
		 */
		 ...
		 *     else if 를 원하는 만큼 반복해서 사용할 수 있으며, 마지막에는 else 를 생략하거나 정의하면 된다.
		 *     
		 *     if(조건식1) {
		 ...
		 *     } else if(조건식3) {
		 *         조건식2의 결과가 거짓이고 데신 조건식3의 결과가 탐일 때 실행할 코드를 여기에 작성한다.
		 *     } ... {
		 *         조건식3의 결과가 거짓이고 대신 생략된 문장안의 조건식의 결과가 참일 떼 실행할 코드를 여기에 작성한다.
		 *     } else { 
		 *         모든 조건의 결과가 거짓일 때 싱행할 코드를 여기에 작성한다.
		 *     }
		 */
		...
		if(num <= 19 ) {
			if(num >= 13 && num <= 15) {
				...
				System.out.println("미성년자 입니다.(고등힉교 졸업생 입니다.)");
			} else {
